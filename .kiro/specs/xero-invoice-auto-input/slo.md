...
- 計測対象: 自動補完 Server Action（`fuzzyMatchAction`）のサーバー処理時間
- 計測方法: Server Action 内の `performance.now()` によるサーバーサイド計測 + ブラウザ Performance API
...
| SLO-005 | Xero API 成功率 | > 99.0% (エラー率 < 1%) | 30日ローリング | REQ: 成功条件「エラー率1%未満」 |
